...
export default function AdminTrips() {
  ...
  const exportPassengersToCSV = async (trip) => {
    ...
    try {
      ...
      const tripName = formatTripTitle(trip.name, trip.start_time || trip.first_time, trip.id).replace(/\s+/g, "_");
  ...
  const selectedTripLabel = formatTripTitle(selectedTripRef?.name, selectedTripRef?.start_time || selectedTripRef?.first_time, selectedTripId);
  ...
  const renderTripCard = (trip) => (
    <div key={trip.id} className="card glass-card stack-sm" style={{ padding: '20px', marginBottom: '16px' }}>
      <div className="row-between">
        <h2 className="headline">{formatTripTitle(trip.name, trip.start_time || trip.first_time, trip.id)}</h2>
      ...
      {reinforcementTargetTrip && (
        ...
        >
          ...
          >
            ...
            <p className="caption">Traslado original: <b>{formatTripTitle(reinforcementTargetTrip.name, reinforcementTargetTrip.start_time || reinforcementTargetTrip.first_time, reinforcementTargetTrip.id)}</b></p>
            <div className="divider" />
